...
<template>
  <Header />
  <main class="min-w-1000 w-full flex flex-col items-center justify-center gap-60 pt-40">
    <div class="w-580 flex justify-end gap-10">
    ...
    <div v-for="(report, index) in props.reportList" class="w-580 min-w-580 flex flex-col cursor-pointer">
        <div class="flex justify-between border-b border-[#3EAE09] text-[#3EAE09] mb-10" @click="emits('handleClickedReport', index, report.attributes.defects.data.length)">
          <span class="min-w-100">{{ new Date(report.attributes.createdAt).toLocaleDateString() }}</span>
          <span class="max-w-252 text-ellipsis overflow-hidden whitespace-nowrap">{{ report.attributes.description }}</span>
