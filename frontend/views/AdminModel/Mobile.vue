...
import AiModel from "../../components/AiModel/Mobile.vue";
...
<template>
  ...
  <main class="flex flex-col items-center justify-center">
    <div
      v-show="isLoading()"
      class="flex my-[200px] items-center justify-center h-8 w-8 animate-spin rounded-full border-4 border-solid border-[#3EAE09] border-current border-r-transparent align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite]"
      role="status"
    >
      <span class="!absolute !-m-px !h-px !w-px !overflow-hidden !whitespace-nowrap !border-0 !p-0 ![clip:rect(0,0,0,0)]">Loading...</span>
    </div>

    <div v-show="isLoading() === false" class="flex items-center w-full">
      <div class="flex-col mt-[20px]">
        <div class="flex flex-row mt-[15px] gap-x-[20px] ml-[20px]">
          <span class="w-[10px] h-[10px] bg-[#93D07F] rounded-full self-center"></span>
          <h2 class="text-[18px] text-[#3EAE09]">현재 서비스 중인 모델</h2>
        </div>
        <section class="w-screen my-[10px] flex flex-col justify-center items-center">
          <div
            v-if="state.current_model"
            :class="{ 'text-[14px] py-[8px]': state.current_model.length > 20 && getScreenWidth() <= 280 }"
            class="w-full max-w-[80%] min-w-[80%] mx-[20px] bg-[#93D07F] h-[40px] py-[5px] text-[18px] text-[#FFFFFF] text-center cursor-pointer"
          >
            {{ state.current_model }}
          </div>
          <div v-if="!state.current_model" class="w-full max-w-[90%] min-w-[90%] h-[60px] py-[5px] text-[16px] text-[#F52E02] text-start ml-[20px]">
            <span class="block">⚠️ 모델 설정 필요.</span>
            <span class="block">아래 목록에서 선택하여 '적용' 버튼을 눌러주세요.</span>
          </div>
        </section>
        <div class="flex flex-row mt-[35px] gap-x-[20px] ml-[20px]">
          <span class="w-[10px] h-[10px] bg-[#93D07F] rounded-full self-center"></span>
          <h2 class="text-[18px] text-[#3EAE09]">전체 모델 목록</h2>
        </div>
        <section class="my-[20px] flex flex-col justify-center items-center">
          <div class="w-[80%] h-[50%] flex flex-col gap-y-[10px] items-center text-[18px] text-[#AFAEAE] overflow-y-scroll">
          ...
          <div class="w-full h-[10%] flex flex-row justify-center items-center mt-[10px]">
            <div class="bg-[#7896FF] w-[80%] h-[40px] py-[5px] text-[18px] text-[#FFFFFF] text-center cursor-pointer hover:bg-[#567BFF]" @click="handleClickAddNewModel">신규 모델 추가하기</div>
          </div>

          <div class="my-[15px]">
            <button class="cursor-pointer my-[20px] mb-[20px] w-[145px] h-[50px] text-[#FFFFFF] text-[18px] bg-[#93D07F] rounded-lg disabled:bg-[#DCDCDC] hover:bg-[#3EAE09]" @click="handleClickRestart">처음으로</button>
          </div>
        </section>
      </div>
    </div>
